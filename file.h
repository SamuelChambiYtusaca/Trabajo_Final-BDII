...
struct FileHeader
{
    ...
    int first_free_pages; // Numero de la primera pagina marcada como "En USO" en el file.
...
public:
    ...
    ~File();                                // Escribe en el .txt
    void open(char separator);              // Abre el .txt y carga todo su contenido
    void remove(std::string &filename);     // Elimina el archivo .txt
    void close(std::string &filename);      // Actualiza el contenido del .txt y cierra el archivo
    bool isOpen(std::string &filename);     // Comprueba si el .txt requerido esta abierto
    bool exists(std::string &filename);     // Comprueba la existencia del .txt
